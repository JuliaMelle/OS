...
function printGanttChart($jobs, $n, $finishTime) {
    for ($i = 0; $i < $n; $i++) {
     echo "Process " . $jobs[$i]->job . " completes at time: " . $finishTime[$i] . "<br>";
    }
   }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (count($arrivalTimes) == count($burstTimes) && count($burstTimes) == count($priorities)) {
     ...
     // Call the scheduling function
$finishTime = priorityScheduling($jobs);
printGanttChart($jobs, $n, $finishTime);
    } else {
